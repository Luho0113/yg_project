...
<html>
  ...
  <body>
    ...
    <div class="content">
      ...
      <form method="POST" action="/myInfo-update" name="update_form" onsubmit="return isThereValue()" enctype="multipart/form-data">
        <!-- 기존 프로필 사진 -->
        <div>
          <img src="${user.uiFilePath}" style="border-radius: 40px" width="80px" height="80px" onerror="this.src='https://ifh.cc/g/cDROLZ.png';" />
        </div>
        <!-- 선택한 프로필 사진 미리보기 -->
        ...
        <div>
          <label>
            <input type="text" name="uiId" id="uiId" value="${user.uiId}" onkeyup="checkId(this)" readonly="readonly" />
          </label>
        </div>

        <!-- 비밀번호 입력 -->
        <div>
          <label> <input type="password" name="uiPassword" id="uiPassword" onkeyup="checkPwd()" placeholder="비밀번호" /> </label>
        ...
        <div>
          <label>
            <input type="text" name="uiNickname" id="uiNickname" value="${user.uiNickname}" onkeyup="checkNickName()" placeholder="닉네임" />
          </label>
        ...
        <!-- 이메일 입력 -->
        <div>
          <label> <input type="email" name="uiEmail" id="uiEmail" value="${user.uiEmail}" onkeyup="checkEmail()" placeholder="이메일" /> </label>
