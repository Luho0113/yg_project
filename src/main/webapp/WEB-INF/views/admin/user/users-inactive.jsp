...
<link rel="stylesheet" href="${path}/resources/css/admin/users-inactive.css">
</head>
<body>
	<!-- header area -->
	<jsp:include page="../header/header.jsp"></jsp:include>

	<div class="content inactiveUser-area">
		<h2>비활성 계정 회원 관리</h2>
	<form action="/admin/inactive-users" method="GET">
		...
		<button class="inactiveUser-search">비활성 회원 검색</button>
	</form>
	<button class="allUserList" onclick="location.href='/admin/users'">전체 회원 관리</button>
	<table class="inactiveUser-table">
	<tr>
		<th><h4>아이디</h4></th>
		<th><h4>닉네임</h4></th>
		<th><h4>프로필 이미지</h4></th>
	...
	<c:forEach items="${page.list}" var="user">
		<tr>
			...
			<td>
			<button class="inactiveUser-update" onclick="location.href='/admin/user-update?uiId=${user.uiId}'">수정</button>
			<button class="inactiveUser-delete" onclick="deleteCheck()">탈퇴</button>
			</td>
		</tr>
		</c:forEach>
	</table>
...
<script>
	...
	let html = '';
	if(start!=1){
		html += '<a href="/admin/inactive-users?page=' + (start-1);
		if('${param.uiId}'){
	...
	for(let i = start; i<=end; i++){
		if(i==page){
			...
			if(i==1){
				html += '<a href="/admin/inactive-users?&uiId=${param.uiId}">[' + i + ']</a>';	
			}else{
				html += '<a href="/admin/inactive-users?page=' + i + '&uiId=${param.uiId}&uiNickname=${param.uiNickname}">[' + i + ']</a>';
			}
		}
	}
		if(end!=pages){
			html += '<a href="/admin/inactive-users?page=' + (end + 1) + '&uiId=${param.uiId}&uiNickname=${param.uiNickname}"></a>';
		}
