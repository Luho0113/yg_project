...
<body>
	...
	<div class="content noticeArea">
		...
		<form action="/admin/notices" method="GET">
			<input type="text" id="niTitle" name="niTitle" placeholder="제목"
				value="${param.niTitle}"> <input type="text" id="niContent"
				name="niContent" placeholder="내용" value="${param.niContent}">
			<button class="noticeSearch">공지 검색</button>
		</form>
		<div class="moveBtn">
			<button class="admin-home" onclick="location.href='/admin/home'">영감
				관리자 홈</button>
			<button class="admin-logout" onclick="location.href='/admin/logout'">영감
				관리자 로그아웃</button>
		</div>
		<button class="registerBtn"
			onclick="location.href='/admin/notice-register'">공지사항 등록</button>
		<table class="noticeTable" border="2">
			<tr>
				<th><h4>번호</h4></th>
				<th><h4>제목</h4></th>
				<th><h4>작성자</h4></th>
			...
			<c:forEach items="${page.list}" var="notice">
				<tr>
					<td>${notice.niNum}</td>
					<td><a href="/admin/notice?niNum=${notice.niNum}">${notice.niTitle}</a>
					</td>
			...
			<script>
				...
				for (let i = start; i <= end; i++) {
					if (i == page) {
						...
						if (i == 1) {
							html += ' <a href="/admin/notices?&niTitle=${param.niTitle}&niContent=${param.niContent}">['
									+ i + ']</a> ';
						} else {
							html += ' <a href="/admin/notices?page='
									+ i
									+ '&niTitle=${param.niTitle}&niContent=${param.niContent}">['
									+ i + ']</a> ';
						}
					}
				}
				if (end != pages) {
					html += '<a href="/admin/notices?page='
							+ (end + 1)
							+ '&niTitle=${param.niTitle}&niContent=${param.niContent}"></a>';
				}
